...
var canvasLength=600, //height and width of Canvas
    validImageTypes=['png','jpeg'], //valid types of images
...
function addIcon(image,fileName){
  ...
  var iconList=document.getElementById('iconList'),
      newIcon=`<li><img id="${fileName}" src="${image}" class="img-rounded" ondblclick="addCanvas('${image}')"></li>`;
...
function addCanvas(image){
  ...
  var id=canvasIndex,
      ...
      newCanvas=`<img class="item" id="${id}" src=${image} ondragstart="dragstart_handler(event, this.id,this.src);" draggable="true">`;
  ...
  state[id]={
    type: 'image',
    max_left: canvasLength-position.width,
    max_top: canvasLength-position.height,
    left:0,
    top:0
  }
